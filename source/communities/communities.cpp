...
#ifndef SCD_MIN_COUNT_TO_PARALLELIZE 
	#define SCD_MIN_COUNT_TO_PARALLELIZE 1 
#endif
...
#define SCD_INVALID_COMMUNITY 0xfffffff
	...
	static Movement CheckForBestMovement( const CGraph* graph, uint32_t node, const CommunityPartition* partition ) {
		...
		double64_t bestInsertImprovement = -10000000000000.0f;
		double64_t bestInsertImprovementExact = -10000000000000.0f;
		...
		for ( uint32_t thread = 0; thread < num_threads; thread++ ) {
			...
			for(uint32_t i = 0; i < numMovements; i++) {
				...
				switch(movement.m_MovementType) {
					case E_REMOVE:
						tempNodeLabels[movement.m_NodeId] = nextLabel;
						removeMovements++;
						nextLabel++;
						break;
	...
	uint32_t 	InitializeSimplePartition( const CGraph* graph, CommunityPartition* partition ) {
		...
		//Creating the community partition.*/
			...
			if(!visited[nodeClustering->m_NodeId]) {
				...
				for( uint32_t j = 0; j < degree; j++ ) {
					if( !visited[adjacencies1[j]] ) {
						const uint32_t* adjacencies2 = graph->GetNeighbors(adjacencies1[j]);
					/*	if( Intersect(adjacencies1, degree, adjacencies2, graph->GetDegree(adjacencies1[j])) > 0 )*/ {

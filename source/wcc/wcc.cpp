...
namespace scd {
	...
	double64_t 	ComputeWCC(const CGraph * graph, uint32_t node, uint32_t communityLabel, const uint32_t * communities, uint32_t communitySize) {
        ...
        if(graph->GetTotalTriangles(node) > 0 && communitySize > 0) {
            return ((internalTriangles / (double64_t)graph->GetTotalTriangles(node)) * (triangleDegree/(double64_t)(triangleDegree - internalTriangleDegree + communitySize - 1)));
        }
		return 0.0;
        }
}
